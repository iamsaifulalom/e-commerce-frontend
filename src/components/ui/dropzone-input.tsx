...
import { UploadedFile, useImageUpload } from "@/hooks/use-image-upload";
...
export default function DropzoneInput({ value, onChange, multiple = false }: DropzoneInputProps) {
  const { files, handleDrop, handleDelete, clearFiles } = useImageUpload({ multiple, onChange });

  useEffect(() => {
    // When react-hook-form resets the 'image' field
